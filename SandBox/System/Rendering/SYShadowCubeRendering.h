...
namespace SandBox {
    class SYShadowCubeRendering: public Scene::SNSystemBase {
        private:
            struct ShadowCubeRenderingInfo {
                struct Resource {
            ...
            } m_shadowCubeRenderingInfo;

        public:
            SYShadowCubeRendering (void) {
                m_shadowCubeRenderingInfo = {};

                auto& logObj = m_shadowCubeRenderingInfo.resource.logObj;
            ...
            void initShadowCubeRenderingInfo (const uint32_t activeLightsCount,
                                              Scene::SNImpl* sceneObj,
                ...
                resource.depthImageObj      = collectionObj->getCollectionTypeInstance <Renderer::VKImage>         (
                    "F_LIGHT_DEPTH_CUBE_0"  /* Use the first depth (cube) image */
                );
                resource.renderPassObj      = collectionObj->getCollectionTypeInstance <Renderer::VKRenderPass>    (
                    "S"
                );
                /* Note that, we are storing frame buffers for every active light (not just the point lights) for ease
                 * of indexing
                */
            ...
            ~SYShadowCubeRendering (void) {
                delete m_shadowCubeRenderingInfo.resource.logObj;
            }
    };
}   // namespace SandBox